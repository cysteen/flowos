...
const RISK_HITS_SEED: RiskHit[] = [
  ...
  //
  // 【为什么单补这一条】「未标记」两路的归属是唯一的（`effectiveSourceOf`：
  // 实时监控 > 重点工单），而「兼：X」标的是这张单**同时满足的其余来源**。
  // 它在两张表上共用同一个判据（`alsoSourcesOf`），可「重点工单」那一路走的是
  // 工作台那张**富列表**，那一路要标出「兼：实时监控」，前提是**该单另有命中记录** ——
  // 而在本批之前那一路的单一条命中都没有，于是这枚小标在富列表上从未渲染过一次：
  // 判据两处共用，样本却只有一处有，等于另一处根本没验过。
  //
  // 挑 `IFLYZX-20260731-00002`（ops-3）是因为它本来就同时满足两路：
  // 条目 `rq-s08` 按优先级从「重点工单」进来（P1 且已超解决时限 79 小时），补上这条命中之后
  // 它**同时**也是预警词那一路的单 —— 归属仍算「重点工单」（条目自带的 source 说了算），
  // 行内多出一枚「兼：实时监控」。
  //
  // ⚠️ **不带 `tagged`**：带了这张单就算下过结论，整行从「未标记」里消失，
  // 而这枚小标只在「未标记」那一段渲染。
  // 🔴 **已核实为误报**（2026-09-15 裁决）：召回清单只列未打标工单上**待核实**的命中，
  // 本单有待核实命中就归「实时监控」。本条误报后本单已无待核实命中，按固定次序改归「重点工单」，
