...
const RISK_HITS_SEED: RiskHit[] = [
  ...
  // ====================================================================
  // 「待标记 · 实时监控」里的**一单多命中** —— 共 3 条（h30 … h32），两组场景。
  //
  // 【为什么要在这一段单独补】文件下方已经有两组一单多命中（h21 / h22 / h23），
  // 但那两组挂的单**都不在 `mock/tickets.ts` 里**，且其中一条已被打过标 ——
  // 它们只出现在「命中台账」那张旁路表上，**「待标记 · 实时监控」这一路一条都没有**。
  // 而实时监控那张表恰恰要按「风险词」「命中原话摘录」两列展开：这两列若每行都只有
  // 一个词、一句话，"一单多命中"这件事在主链路上就无从表达，`presetLevelOf` 的
  // **取最重**规则也演示不到（一张单只有一条命中时，取最重恒等于取那一条）。
  //
  // 🔴 两组各演一件事，**别合成一组**：
  //   · A 组（h30 + h25 + h31，同挂 t9）——**三条不同等级的词、按时间逐级爬坡**：
  //     低「差评」→ 中「投诉到底」→ 高「12315」。这一单的识别等级因此是**高**，
  //     取的是最重那条而不是最新那条、也不是第一条。这是取最重规则唯一说得清的样本。
  //   · B 组（h26 + h32，同挂 t27）——**最重的那条不是最新的那条**：
  //     先中「退一赔三」、后低「差评」，识别等级仍是**中**。少了它，人容易把
  //     "取最重"读成"取最后一条"，A 组恰好两者同解，分不开。
  //
  // ⚠️ 这三条与上面 h24…h29 受同一批约束：单必须真实在办且尚未打标，
  // `when` 不得晚于当前最新的一条（h1 的 08-04 14:21），否则 `anchorHitDates()`
  // 的锚点会被挪走。
  // ====================================================================
  {
    // A 组第一句：最早、最轻。客户只是撂了句差评，尚未提任何外部渠道。
    id: 'h30', ticketNo: 'IFLYZX-20260715-00003', title: '空气净化器滤芯指示灯常亮',
    word: '差评', matchedWord: '给差评', level: '低', position: '沟通记录',
    excerpt: '换了滤芯灯还亮，你们这产品我只能给差评了。',
    when: '2026-08-04 09:30', customer: '李静',
    groupId: 'cs-1', groupName: '受理一组', assignee: '陈坐席',
    receivers: ['值班经理'],
    speakerRole: '客户',
    impact: '一般', impactSource: '产品功能/性能投诉 · 产品性能未达到顾客预期',
  ...
  {
    // A 组第三句：最晚、最重，且**指名了外部渠道**。本单的识别等级由它定成「高」——
    // 中间那句「投诉到底」（h25）与最早那句「差评」（h30）都不参与定级，但都要显示：
    // 判这一单的人要看见的是这三句连起来的爬坡，不是最重的那一句孤零零摆着。
    id: 'h31', ticketNo: 'IFLYZX-20260715-00003', title: '空气净化器滤芯指示灯常亮',
    word: '12315', matchedWord: '12315', level: '高', position: '催补记录',
    excerpt: '第三次催了还是没人上门，今天不给答复我就打 12315。',
    when: '2026-08-04 13:40', customer: '李静',
    groupId: 'cs-1', groupName: '受理一组', assignee: '陈坐席',
    receivers: ['值班经理', '投诉专员'],
    speakerRole: '客户',
    impact: '一般', impactSource: '产品功能/性能投诉 · 产品性能未达到顾客预期',
  ...
  {
    // B 组第二句：**比 h26 晚、却比 h26 轻**。本单识别等级仍由 h26 的「中」定，
    // 不因为它是最新的一条就降到「低」——"取最重"与"取最新"在这一单上分得开。
    id: 'h32', ticketNo: 'IFLYZX-20260710-00002', title: '路由器固件升级后无法联网',
    word: '差评', matchedWord: '评价里说', level: '低', position: '沟通记录',
    excerpt: '这次要是还修不好，我就在评价里说清楚你们的升级是怎么把机器搞坏的。',
    when: '2026-08-03 17:10', customer: '田军',
    groupId: 'cs-2', groupName: '受理二组', assignee: '陈坐席',
    receivers: ['值班经理'],
    speakerRole: '客户',
    impact: '一般', impactSource: '产品质量投诉 · 产品质量故障',
    signal: '无', signalSource: '命中「差评」（同义词「评价里说」）',
    track: '外部信号', legacyLevel: '低',
  },

  // ====================================================================
  // 「已标记 · 已入池」那张表的**证据语料** —— 共 3 条（h33 … h35）。
  //
  // 【为什么必须补】「已标记」段的两列是**按行分岔**的（见 `RiskMonitorView` 的
  // `rowHasHits`）：有命中记录的行摆「风险词 + 命中原话摘录」，没有的摆工单自己的问题描述。
  // 而在本批之前，池内那十几条条目里**只有 rr-010 一条**（`IFLYTS-20260731-00001`，靠 h5）
  // 走得到前一支 —— 一条样本证明得了这个分支存在，却证明不了它有内容：
  // 翻一页表全是后一支，"复核一条打标结论时能看见原话"这件事在页面上基本发生不了。
  //
  // 🔴 **这三条一律不带 `tagged` / `verdict`**（＝仍在待核实）。打标与命中核实是两条线：
  // 条目那边早已有结论（低 / 中 / 高），命中这边的核实答的是"**规则这次捞得准不准**"，
  // 由命中台账回填。顺手给它们补一个 `tagged` 会让工单级等级凭空多出一个来源，
  // 而那个等级本该只由条目的打标给（见 `riskTags.ticketGradeOf`）。
  //
  // ⚠️ 三条的措辞与各自条目的打标结论**对得上**：条目判中危的，命中就该是"有威胁未指名"
  // 那一档的词；判高危的，命中里得真出现外部渠道。对不上的话，这一列摆出来反而
  // 给复核的人一个自相矛盾的证据。
  // `when` 与上面几批受同一条约束：不得晚于当前最新的一条（h1 的 08-04 14:21）。
  // ====================================================================
  {
    // 条目 `rq-s09`（投诉单 · 打标中危 · 吴投诉）的证据。
    // 打标注记写的是「客户要求书面答复但未提及外部渠道」，故这里取「投诉到底」这一档：
    // 有威胁、未指名渠道，signal ＝ 中，与那句注记逐字对得上。
    id: 'h33', ticketNo: 'IFLYTS-20260817-00002', title: '坐席态度问题投诉',
    word: '投诉到底', matchedWord: '投诉你们', level: '中', position: '沟通记录',
    excerpt: '这个态度我实在不接受，今天不给我一个书面答复，我就一直投诉你们。',
    when: '2026-08-04 10:35', customer: '郑霞',
    groupId: 'cs-1', groupName: '受理一组', assignee: '王坐席',
    receivers: ['值班经理'],
    speakerRole: '客户',
    impact: '一般', impactSource: '服务过程投诉 · 服务态度差（辱骂、推诿、抢话等）',
    signal: '中', signalSource: '命中「投诉到底」（同义词「投诉你们」）',
    track: '外部信号', legacyLevel: '中',
  },
  {
    // 条目 `rr-011`（投诉单 · 打标中危 · 评估中）的证据。
    // 这一条同时是「已领取」那一档里唯一带原话的行 —— 三个处置阶段各有一行带证据，
    // 证据列才不会只在某一个阶段里出现。
    id: 'h34', ticketNo: 'IFLYTS-20260730-00001', title: '学习机批量激活失败未解决',
    word: '投诉到底', matchedWord: '投诉到底', level: '中', position: '催补记录',
    excerpt: '开学就在下周，320 台到现在还是激活不了，再没个结果我就投诉到底。',
    when: '2026-08-04 08:20', customer: '赵敏',
    groupId: 'edu', groupName: '教育支持组', assignee: '孙坐席',
    receivers: ['值班经理'],
    speakerRole: '客户',
    impact: '一般', impactSource: '产品功能/性能投诉 · 产品性能未达到顾客预期',
  ...
  // ====================================================================
  // 「兼：X」（多路来源行内小标）在**富列表那一路**的样本 —— 1 条（h36）。
  //
  // 【为什么单补这一条】「未标记」三路的归属是唯一的（`effectiveSourceOf`：
  // 实时监控 > 投诉单 > 重要紧急），而「兼：X」标的是这张单**同时满足的其余来源**。
  // 它在两张表上共用同一个判据（`alsoSourcesOf`），可「投诉单」「重要紧急」两路走的是
  // 工作台那张**富列表**，那两路要标出「兼：实时监控」，前提是**该单另有命中记录** ——
  // 而在本批之前那两路的单一条命中都没有，于是这枚小标在富列表上从未渲染过一次：
  // 判据两处共用，样本却只有一处有，等于另一处根本没验过。
  //
  // 挑 `IFLYZX-20260731-00002`（ops-3）是因为它本来就同时满足两路：
  // 条目 `rq-s08` 从「重要紧急」进来（P1 且已超解决时限 79 小时），补上这条命中之后
  // 它**同时**也是预警词那一路的单 —— 归属仍算「重要紧急」（条目自带的 source 说了算），
  // 行内多出一枚「兼：实时监控」。
  //
  // ⚠️ **不带 `tagged`**：带了这张单就算下过结论，整行从「未标记」里消失，
  // 而这枚小标只在「未标记」那一段渲染。
  // 🔴 **已核实为误报**（2026-09-15 裁决）：召回清单只列未打标工单上**待核实**的命中，
  // 本单有待核实命中就归「实时监控」。本条误报后本单已无待核实命中，按固定次序改归「重要紧急」，
  // 「兼：实时监控」照旧（本单在命中台账里有记录）。
  // ====================================================================
  {
    id: 'h36', ticketNo: 'IFLYZX-20260731-00002', title: '开放平台配额申请无人跟进',
    word: '差评', matchedWord: '评分给低', level: '低', position: '沟通记录',
    excerpt: '配额申请交上去三天没人接，这次的服务评分我只能给低了。',
    when: '2026-08-04 11:30', customer: '韩雪',
    groupId: 'tech-1', groupName: '技术支持组', assignee: '周工',
    receivers: ['值班经理'],
    speakerRole: '客户',
    impact: '轻微', impactSource: '流程规则投诉 · 联系/响应时效规则不认可',
    signal: '无', signalSource: '命中「差评」（同义词「评分给低」）',
    track: '外部信号', legacyLevel: '低',
    taggedBy: '秦督导', taggedByRole: '投诉督导', taggedAt: '2026-08-04 11:52', taggedNote: '客户催的是配额审批进度，服务评分是对响应时效的反馈，未指向投诉或外部渠道',
    verdict: '误报',
  },
 ...
 * 风险监控页头就会天天给出「今日发现 12 / 今日命中记录 0」这种自相矛盾的读数——
