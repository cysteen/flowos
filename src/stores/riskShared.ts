...
/**
 * **A 线的自动识别来源**（2026-09-18 裁决 · 漏斗的入口）。**两类、两个字面量**：
 *   ① **实时监控** —— 预警词命中。
 *   ② **重点工单** —— 在办 ∧（工单类型 ＝ 投诉 ∨ 优先级 ∈ {P0, P1}）自动纳入监控。
 *
 * 🔴 **原「投诉单」「重要紧急」两路已合并成「重点工单」一路**：两路的行都是工单、
 * 列与筛选逐字相同、子档都按工单优先级分，摆成两档只是把同一件事切成两半 ——
 * 而切开的那条线（是不是投诉）恰恰已经由「原单类型」这一维单独答过了。
 * 合并之后来源这一维只剩"这条是靠词捞进来的，还是靠单本身的分量捞进来的"两个答案。
 * 「投诉单」「重要紧急」作为**工单类型 / 优先级**的词照旧用，只是不再是来源取值；
 * 旧数据里落的这两个来源值由 `normalizeMonitorSource` 一律读成「重点工单」。
 *
 ...
 */
export const QUEUE_SOURCES = ['实时监控', '重点工单'] as const;
...
/**
 * 合并池那一列「监控来源」的取值全集 ＝ A 线两类 + B 线一枚。
 *
 * ⚠️ **这是过渡层的口径**：风险工单池今天还把两条线装在一张表上，来源 chip 那一排要把
 * 这几个数并排摆出来，故仍需要一个并集。下一批把两池拆开之后，A 线的页面只读 `QUEUE_SOURCES`，
 ...
 *
 * ⚠️ **它已经不再是"要不要打标"的判据**：新口径下 A 线**两类来源都要打标**才进池
 * （打标是入池门槛，见 `RiskTagRecord`）。本函数现在只回答"这条是不是预警词捞进来的"，
 * 用处是页面上要把命中原话摆出来 —— 「重点工单」那一路没有原话可摆。
 */
 ...
 *
 * 🔴 「投诉单」「重要紧急」在 2026-09-18 之前是两个**正式来源取值**，现已并成「重点工单」。
 * 存量缓存（条目与池行）里落的还是旧值，界面与筛选一律按新值走，故这两条映射不能省：
 * 少了它们，旧缓存里那批条目会被 `isQueueSource` 判成非法值整批丢掉，或在来源 chip 那一排
 * 长出两枚点不亮的幽灵档。`全量投诉` / `紧急重要` 是更早一代的写法，一并收在这里。
 */
...
export interface RiskTagRecord {
  ...
  /**
   * 本条打标记录**由命中核实**产出（2026-09-15 裁决）：未打标工单上一条命中首次核实为成立 → 打为所选等级；
   * 或全部命中核实为误报且不改归「重点工单」 → 打为无风险。只挂在那一次写入的记录上，
   * 之后在「已标记」段修正产出的记录不带它。
   */
...
/**
 * **一次「释放」的留痕**（《【930】》§5.5 ⑥ · v3.5 新立的动作）——
 * 承办人把已领取的条目退回「待领取」，条目上累积这么一条。
 *
 * 【为什么必须留痕、而且必须累积】释放把条目**退回池子重新可领**，同一条被领取释放 N 次
 * 就有 N 条记录（§5.5 ⑥ 原话「累积不覆盖」）。只改状态不留痕的话，条目回到「待领取」之后
 * 界面上与"从来没有人领过"完全一样 —— 而这两件事对下一个来领的人意义相反：
 * 后者是新活，前者是**别人看过之后退回来的活**，退回的理由恰恰是他要先读的东西。
 *
 * 【为什么不复用 `ReportAssessment` / `RiskCoordination`】那两个是**结论**（条目就此出列），
 * 释放是**回退**（条目留在队里、等待时长照走）。形状相近但生命周期相反：结论至多一条、
 * 提交即固化；释放可以有很多条，且每一条之后条目都还会被再领一次。
 *
 * 🔴 **不落《【720】》第八类履历**（§5.5 ⑦）：释放与领取同属队列内部事件，工单一格没动。
 * 🔴 **本轮不发通知**（§5.5 ⑧）：收件人口径已预先写定（B 线＝报备人、A 线＝不发，
 * 与 `risk.report.claimed` 对称），但本册不新增任何通知事件，见 `stores/riskPool.ts` 的 `release`。
 */
export interface RiskReleaseRecord {
  /** 释放人姓名 */
  by: string;
  /** 释放人角色。**必须留**：§5.5 ② 里承办人本人与管理员兜底是两条不同的口子，
   * 只记名字的话，事后分不出这一条是"自己退回来的"还是"管理员替他退回来的" */
  byRole: string;
  /** 释放时刻 */
  at: string;
  /** 释放原因（必填，§5.5 ④）。空白与全空格由调用方拦下 */
  reason: string;
}
...
export interface RiskPoolItem {
  ...
  /**
   * 历次**释放**记录，时间正序、**累积不覆盖**（§5.5 ⑥）。**两条线都有** ——
   * 释放是两个池同一套口径（§5B.4「逐条同 §5.5」），故它住在共享层而不是某一条线里。
   *
   * ⚠️ **空数组与 undefined 同义**（"没有被释放过"）：旧缓存里没有这一格，读进来即 undefined，
   * 调用方一律 `releases ?? []`。**不要为了"整齐"给它补一个空数组默认值** —— 那会让
   * 每一条种子在缓存里都多写一格，而这一格的含义本来就由"有没有"承担。
   */
 ...
 *
 * ⚠️ **已知取舍 · 凌晨看到的等待时长会偏短，那不是算错了**（2026-09-11 拍板：接受，不改）。
 * 走本函数生成的那批条目，白天在队列里显示「已等待 65 / 88 / 110 分钟」，
 * 凌晨 00:26 打开却只有几分钟。**这是两个口径在凌晨的物理冲突，不是缺陷**：
 * 一条条目不可能在 00:26 就已经"**今天**等了 65 分钟"——今天总共才过了 26 分钟。
 * 「落在今天」与「等了很久」在那个时刻无法同时为真，只能二选一：
 *   · 保「等了很久」→「今日发现 / 今日打标 / 今日已结论」整排归零（就是本次要修的那个 bug）；
 *   · 保「落在今天」→ 凌晨的等待时长偏短，而**真正要演示超时的那几条已单独留给 `agoStamp`**，
 *     「超时未评」那一档不受影响。
 * 故选后者。系数在 06:00 之后恒为 1，真实演示时段（工作日白天）两者完全等价，
 * 这个偏短只出现在没人演示的凌晨几小时里。**看到短等待时长请不要"顺手修正"它。**
 */
...
/**
 * ⚠️ **已停止作为报备通知的收件人使用**（2026-09-11，基线 v1.24）。
 *
 * 它是分派时代的产物：那时报备提交后要由投诉督导分派给客诉专员，故队列的第一收件人是他。
 * **分派 / 改派 / 批量分派整套已取消**（业务第三轮拍板），两个池子都只留自取，
 * 而基线 v1.24 明确「两个池上管理员与客诉专员同权」—— 报上来要惊动的是**能领这条的人**，
 * 见下方 `RISK_POOL_RECEIVERS`。督导本轮已去权（只看数据、不出动作），
 * 把队列通知发给一个动不了它的人，等于这条通知没有落点。
 *
 * 【为什么留着这个常量】投诉督导在别处仍是一个正式角色名（页签可见性、看板口径）；
 * 删掉它只是把同一个字符串挪到别处再写一遍。**但不要再把它塞进任何 `receivers`**。
 */
export const RISK_SUPERVISOR = '投诉督导';

/**
 * **两个风险池的收件人 ＝ 客诉专员 + 管理员**（基线 v1.24：两池上二者同权）。
 *
 * **按角色而不是按人**：领取职责挂在岗位上，谁在岗谁收，不能写死某个人名 ——
 * 他休假那天这条队列就没人管了。这也正是"谁有空谁领"这条口径在通知侧的对应物：
 * 队列不再有单点，故通知也不该只发给一个人。
 */
export const RISK_POOL_RECEIVERS: string[] = ['客诉专员', '管理员'];
 ...
 * 「今日发现 / 今日打标 / 今日已结论」与「已结论·仅今日」当场归零 ——
