 ...
 *   · **提交时刻 `at`** 默认 `todayStamp`（不跨零点，落在今天），
...
const SEED: RiskReport[] = [
   ...
   * 投诉单不做风险评估、只走协同处理（A 线 rq-s14 / rq-s20），故没有"投诉单上的升级"样本；
   * A 线的升级样本是 rr-009（咨询单 rk-5 → 派生投诉单，见 `stores/derivedTickets.ts` 的 `SEED_RISK_ESCALATION`）。
   */
  reported({
    id: 'rr-006',
...
export const useRiskReportStore = defineStore('riskReports', () => {
   ...
   * （客诉专员在一张 P0 / P1 的咨询单上评的，正是 A 线的条目）。
   *
   ...
   * 而 A 线的条目**不是谁报上来的** —— 它由系统按两类判据自动捞进来，没有报备人、
   * 没有报备原因。让它占住这个名额，后果是：
