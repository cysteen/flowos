 ...
 *   · **非投诉单** —— 处理人不能打标，靠命中规则或**审核人员在后台打** → 两位投诉督导。
 ...
 * | 已标记 · 等级 | `tag.result` | 高 5 / 中 4 / 低 3 |
 * | 已标记 · 按标记人 | `tag.by` | 郑监控 6 / 吴投诉 4 / 秦督导 2（见 `SEED_TAGGERS`） |
 * | 已标记 · 无风险 | 「已标记无风险」态 | 4 条 |
 * | 待处置 · 三态 | `status` | 待领取 5 / 已领取 3 / 已结论 4 |
...
const SEED: RiskQueueEntry[] = [
  ...
  autoEntry({
    ...
    source: '实时监控',
    desc: '沟通记录命中风险词「投诉到底」，已自动纳入实时监控，待打标。',
    at: agoStamp(52),
    status: '实时监控中',
  }),
  autoEntry({
    ...
    source: '实时监控',
    desc: '沟通记录命中风险词「退一赔三」，已自动纳入实时监控，待打标。',
    at: agoStamp(145),
    status: '实时监控中',
  }),
  autoEntry({
  ...
  /* ==================================================================
   * 视图二：已入池（打标为低 / 中 / 高）—— 12 条
   *
   * 三态各有样本（待分派 5 / 评估中 3 / 已评估 4），等级三档各有样本（高 5 / 中 4 / 低 3），
   * 打标人三个（郑监控 6 / 吴投诉 4 / 秦督导 2），已评估那四条把**三种收口方式**
