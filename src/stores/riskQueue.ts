 ...
 *
 * 【这条线现在长什么样】两类自动识别 → **实时监控** → **风险打标** → 风险工单池：
 *
 * ```
 * 预警词命中 / 重点工单  ── 两类自动进 →【实时监控】
 *                                     │
 *                               风险打标
 *                                     │
 *               低 / 中 / 高 ─→ 出实时监控，进【风险工单池】
...
/**
 * 预置数据。三条视图各有样本（待打标 / 已入池 / 已标记无风险），且两类自动识别来源各有条目。
 *
 ...
 * | 待标记 · 重点工单 | 工单**优先级** | 由工单库直接决定，本文件只负责不把它们清空 |
 ...
 *   · **进队时刻 `at`** —— 默认 `todayStamp`（保证落在今天，页头「今日发现」才有数），
 *     **唯独三条超时样本（rr-011 / rq-s13 待领取、rq-s12 评估中）用 `agoStamp`**：它们是「超时未评 3」的全部来源，
...
const SEED: RiskQueueEntry[] = [
   ...
   * 故它既要按来源铺开（两类来源各有条目），也要按班组铺开（五个班组各有条目）。
   * ================================================================== */
  ...
  /*
   * 「重点工单（投诉类）· 待打标」样本。**必须有这一条**：旧口径下投诉单是**直接进池**的，
   * 新口径把它也拦在打标这道门前。少了它，"两类来源都要打标"这句话在页面上
   ...
   *   · t29『屏幕花屏需返厂检测』——投诉单、P0、未认领，按**类型**进「重点工单」；
   *   · ops-3『开放平台配额申请无人跟进』——非投诉、P1、已超解决时限 79 小时，
   *     按**优先级**进「重点工单」。两条一起证明这一路的判据是"类型 ∨ 优先级"，不是单一条件。
   ...
   * 照现场升级的 `deriveEscalatedComplaint` → `ensureEntryFor` 落一条：来源②重点工单、落「实时监控中」、
   * 进监控时刻 ＝ 派生时刻（评估时刻）。`desc` 与 `AUTO_DESC.重点工单` 同一句（该常量在本数组之后声明，这里写字面量）。
   */
  autoEntry({
    id: 'rq-s22',
    ticketNo: SEED_RISK_ESCALATION.no,
    source: '重点工单',
    desc: '在办投诉类工单或 P0 / P1 工单，自动纳入实时监控，待打标。',
  ...
  /*
   * 「重点工单（非投诉 P0）· 打标高危 → 待分派」样本。**必须有这一条**：两类来源里
   * 只有它一条样本都没有，于是来源 chip 行那一枚恒为 0、按「监控来源」列筛选与排序时它永远是空的。
   *
   * 挑这张单是因为它自己就是「重点工单」按优先级那一支的判据本身：P0 + 已超解决时限 41 小时 +
  ...
  /*
   * 「重点工单（投诉类）· 打标中危 → 待分派」。落款吴投诉（客诉专员）：投诉单由客诉专员在
   ...
   *
   * 挑 t14『扫地机器人充电故障』：非投诉、P1、未认领，按优先级进「重点工单」；
   ...
   * ---- 投诉单 · 待领取 · 已超处置时限 2 条 ----
   *
   * 🔴 **投诉单条目不经领取，没有「已领取」态**（《【930】》§5.4 ⑥）：池行只摆「协同处理」，
   * 首次协同直接转「已结论」。故这两条投诉单超时样本落「待分派」（界面词「待领取」）、不带承办人。
   * 超时按进监控时刻算、与领取无关，两条仍计入「超时未评 3」。
   *
   * 挂 `IFLYTS-20260730-00001`（ops-2：投诉、P0、VIP 校长、已超解决时限 88 小时）与
   * `IFLYTS-20260802-00001`（主刷电机异响两次上门未解决），真实存在，且与本线其余条目分开挂，
   * 避开"同单至多一条在队"。240 / 165 分钟前进的监控：已过 2 小时时限。
   */
  autoEntry({
    id: 'rr-011',
    ...
    // 🔴 **进队时刻走 `agoStamp` 而不是 `todayStamp`**：本条与 rq-s13、rq-s12 三条是
   ...
   * 领取这个动作做完是什么样子在开屏时看不到（B 线的种子里也没有评估中的）。
   *
   * 🔴 **必须是非投诉单**：投诉单条目没有「已领取」态（§5.4 ⑥）。
   * 挂 `IFLYZX-20260713-00001`（咨询、P1），205 分钟前进的监控：已过 2 小时时限，演示「已领取也会超时」。
   */
  autoEntry({
    id: 'rq-s12',
   ...
   * 顺带也保证了页头「今日发现」不把它们数进去 —— 那一格问的是今天的入口流量。
   */
  /*
   * 【收口方式 ③：协同处理 · 昨天】投诉单那一路。挑 t12『退款迟迟未到账』：
   * 投诉、P0、在办、受理一组，类型与优先级两支都踩在「重点工单」的判据上。
   */
  autoEntry({
    id: 'rq-s20',
 ...
 *     且开屏按当时的三类判据**补齐监控条目**（`syncAutoEntries`），「未标记」段不再有无条目的行。
 ...
 *     种子新增 `rq-s23`（t42，h2 已核实成立 · 高 → 待分派）；命中表 `h36` 补为误报（ops-3 仍归工单属性那一路）。
 *     v14 那份里 t42 没有条目，工单级等级已是高危却不在池里。
 *   · v15 → v16：**来源枚举由三类并成两类**（2026-09-18 裁决，值域变了）：「投诉单」「重要紧急」
 *     合并为「**重点工单**」＝ 在办 ∧（类型 ＝ 投诉 ∨ 优先级 ∈ {P0, P1}）。种子里那 20 条
 *     旧来源值一并改写，`AUTO_DESC` 由三句收成两句。
 *     v15 那份缓存里躺着 `source: '投诉单' / '重要紧急'`，两者都已不在枚举里 ——
 *     不升号的话，读进来要么被 `isQueueSource` 整批判掉（左栏当场少一大截），
 *     要么在来源 chip 那一排长出两枚点不亮的幽灵档。
 *     🔴 **归一化（`normalizeMonitorSource`）仍保留这两条映射**：版本号管的是本线自己的缓存，
 *     而池行、工单页等处还会读到别处传来的旧值，两道各拦各的，见 `riskShared.LEGACY_MONITOR_SOURCE`。
 */
...
const AUTO_DESC: Record<QueueSource, string> = {
  ...
  重点工单: '在办投诉类工单或 P0 / P1 工单，自动纳入实时监控，待打标。',
};
 ...
 *   · `rerouted` —— 全部误报，改归「重点工单」，仍在「未标记」；
 *   · `noRisk`   —— 全部误报且不符合另一路判据，工单打为无风险。
 */
...
export const useRiskQueueStore = defineStore('riskQueue', () => {
   ...
   * **投诉单本身就是风险信号**，优先级只决定它排多前）；非投诉单则以优先级立身，P0 / P1 的门照旧。
   * 原「投诉单」「重要紧急」两个来源取值即由这两支合并而来。
   *
   * ⚠️ **必须与界面侧的 `RiskMonitorView.effectiveSourceOf` 逐条同源**。此前这两处分过家：
   * 界面判「类型＝投诉」、入队判「类型＝投诉 ∧ P0/P1」，于是一张 P2 的投诉单
   * **在列表里算一档、却进不了队**（`ensureEntryFor` 判它"不在自动识别范围内"）。
  ...
  function tagBlockReasonOf(ticketNo: string): string {
    ...
    return known
      ? '本单不在实时监控的两类自动识别范围内（无预警词命中，也不是在办的投诉单或 P0 / P1 单），不能在工单页打标'
      : '工单库里查不到本单，无法判断它属于哪一类监控来源';
  }

  /**
   * **拿到一条可打标的条目**：本单已有条目就用它，没有就**按两类判据现补一条**。
   *
   ...
   * 满足两类判据之一」、却还没有任何 A 线条目的单，各补一条落「实时监控中」的条目。
   *
   ...
   * 进监控时刻：
   *   · 预警词那一路取本单**最早一条命中的时刻**；
   *   · 重点工单那一路（以及取不到命中时刻的）取**这条条目被补建的时刻**，不取建单时刻 ——
   ...
   *   误报 · 工单未打标 · 已无待核实命中 ──▶ 按 `autoSourceFor` 的固定次序改归「重点工单」（仍未标记）；
   *                                        推不出来 → 打为无风险（记「由命中核实」）
   * ```
   ...
   *
   * 🔴 **不再按来源过滤**：新口径下两类来源都要打标才进池，
   * 旧实现里那道 `isVerifyMonitorSource` 的门在新模型下会把「重点工单」那一路永远挡在池外。
   *
  ...
  // 开屏补齐一遍：种子与缓存之外，工单库里满足两类判据的在办单都要有条目（见 `syncAutoEntries`）。
