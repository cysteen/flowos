...
export interface RiskQueueEntry {
  ...
  tag?: RiskTagRecord;
  /**
   * 进池之后的评估结论（不升级 / 接管）。动作在合并层 `stores/riskPool.ts`，两条线共用。
   */
 ...
 * 本线的缓存键与格式版本。
 *
 * 🔴 **版本必须从 1 升到 2**：v1 那份缓存里躺着来源为「VIP客户」的条目与旧的
 * 「一进来就待分派」状态模型。不升版本，旧缓存会把一批**没有 `tag` 却落在「待分派」**的条目
 * 读进来 —— 它们会直接出现在风险工单池里，而漏斗的门槛正是"有 `tag` 才进池"，
 * 池子当场自相矛盾。版本对不上一律丢弃，见 `readRiskCache`。
 */
