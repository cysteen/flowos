...
export const useRiskTagStore = defineStore('riskTags', () => {
   ...
   *
   * 【max 取在哪一维、不取在哪一维 —— 两句要一起读】
   *   · **跨条目 / 跨命中取 max**：本单有几条结论就比几条。后到的一条低危不该把另一条已判高危的
   *     单子拉回低危 —— 等级降下来了，风险没有降。这一维是棘轮。
   *   · **同一条内以最新结论为准，不是棘轮**：`tagGrades` 每个条目只占一格，改判覆盖那一格
   *     （见上面 `tagGrades` 的说明）。改判是**人做出的降级判断**、还必须填改判理由；
   *     把它挡住等于让"改判"永远改不动工单级等级，那条功能当场作废。何况改判为「无风险」
   *     本来就会清掉那一格 —— 真做成只升不降，这一档先自相矛盾。
   *
   * ⚠️ 界面上曾统一写作「取 max、只升不降」：那句话只说了第一维，读起来像是连改判也不降，
   * 与实现和业务本意都对不上（连改四次 无风险→低→高→中，界面写着只升不降、实际显示中危）。
   * 文案已按上面两句改；**改这里的口径必须回头改那几处文案**：`OpRiskMonitorTab.vue`
