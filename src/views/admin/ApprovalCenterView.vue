    ...
    >
      <template v-if="current">
        <div class="detail-meta">
          <div><span class="lbl">申请编号：</span><span class="mono">{{ current.code }}</span></div>
          <div><span class="lbl">申请人：</span>{{ current.applicant }}</div>
          <div><span class="lbl">工单类型：</span><strong>{{ current.typeName }}</strong></div>
          <div><span class="lbl">申请时间：</span>{{ current.time }}</div>
        </div>

        <div class="detail-section">
          <span class="sec-title">风险等级</span>
          <a-tag :color="riskColor[current.risk]">{{ current.risk }}风险</a-tag>
        </div>

        <div class="detail-section">
          <div class="sec-title">变更详情</div>
          <div class="diff-list">
            <div class="diff-add">+ 新增字段：客户满意度评分（1-5星）</div>
            <div class="diff-mod">~ 修改流程：增加「质检审核」节点</div>
            <div class="diff-del">- 删除字段：旧版备注字段</div>
          </div>
        </div>
        ...
        <div v-if="current.status === '待审批'" class="detail-section">
          <div class="sec-title">审批意见</div>
          <a-textarea v-model:value="comment" :rows="3" placeholder="填写审批意见..." />
        </div>
...
.diff-list { display: flex; flex-direction: column; gap: 6px; font-size: 13px; font-family: ui-monospace, monospace; }
...
.impact-value { font-size: 22px; font-weight: 700; margin: 4px 0; }
