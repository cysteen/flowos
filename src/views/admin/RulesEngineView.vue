...
const WORKORDER_COMMON_FIELDS: LibField[] = [
  ...
  { key: '前期反馈', label: '前期反馈', type: '枚举', group: '投诉专属' },
...
const FIELD_ENUM_VALUES: Record<string, string[]> = {
  ...
  // 状态＝基线 §1 的 25 个可落库子状态 + 刷机单「自动刷机中」（顺序同 BASELINE_STATUSES）；条件判断一律用子状态，不用粗粒度分组
  状态: [
    '草稿', '未认领', '待响应', '处理中', '调研中',
  ...
  投诉类型: ['投诉', '举报', '信访件', '督办件', '监测线索', '监管同步', '其他'],
  投诉平台: ['黑猫投诉', '12315', '工信部', '微博', '抖音'],
  归属业务线: ['消费者BG', '教育BG', '开放平台'],
  前期反馈: ['是-400', '是-线上店铺', '是-线下店铺', '是-业务渠道', '否'],
  服务回溯: ['已回溯', '未回溯', '无需回溯'],
  投诉一类: ['服务态度', '处理时效', '解决方案', '承诺未兑现'],
  投诉二类: ['响应慢', '推诿', '重复来电', '其他'],
  客户类型: ['个人', '企业', '渠道', 'VIP'],
  客户等级: ['普通', '银卡', '金卡', '钻石'],
  // 风险等级：取值与数据字典 `risk_level` 逐值一致（低/中/高），代码侧不得自造。
  // 【为什么只有三档】"有没有风险"是另一个字段（`risk_flag`）承载的，不是本刻度的一档——
  // 把"无"塞进等级里，会让"判为误报"和"风险很低"变成同一个值，跨页对账时永远分不开。
  // 要筛"无风险的单"请用 `risk_flag`，别在这个取值域里找。
  // 【「极高」为何删】它是 2026-07-09 一次与风险无关的提交（软电话拨号盘 + SLA 双时钟）里顺手补的，
  // 当时风险监控模块还不存在；全仓零使用（无规则、无模板、无数据引用），2026-08-28 删除。
  风险等级: ['低', '中', '高'],
...
const TICKET_TYPES = ['投诉', '建议', '商机', '咨询', '刷机'];
const ESC_TYPES = ['技术问题', '产品缺陷', '紧急事项'];
const routingMatrix: Record<string, Record<string, string>> = {
  学习机: { 咨询: '学习机一线组', 投诉: '学习机处理组', 建议: '学习机建议组', 商机: '商机跟进组', 刷机: '一线刷机池' },
  翻译机: { 咨询: '翻译机一线组', 投诉: '翻译机处理组', 建议: '翻译机建议组', 商机: '商机跟进组', 刷机: '一线刷机池' },
  录音笔: { 咨询: '通用一线组', 投诉: '通用处理组', 建议: '产品建议组', 商机: '商机跟进组', 刷机: '一线刷机池' },
  智能办公本: { 咨询: '通用一线组', 投诉: '通用处理组', 建议: '产品建议组', 商机: '商机跟进组', 刷机: '一线刷机池' },
};
