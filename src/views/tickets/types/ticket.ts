 ...
 *
 * 可执行领取 / 评估 / 释放的角色 ＝ **客诉专员 + 三类管理员兜底**（与风险监控页风险工单池同口径）。
 * ① 报备单的评估结论由**客诉专员**给出（《【930】》§5.4，评估二选一）；
 * ② **投诉督导已去权**（业务第三轮拍板）：页签可见、只看数据，行内动作列整列为「—」。
 *
 * ⚠️ **本函数同时决定报备池行内动作露不露出来**：返回 false 的角色（投诉督导）整列为「—」，
 * 待领取行也不出「领取」按钮（与 A 线风险工单池的 `canClaim` 同形，不是露出来再点一下拦截）。
