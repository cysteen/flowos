...
const TAB_TYPE_RESTRICTION: Partial<Record<ProcessTabKey, string[]>> = {
  ...
  // 但这个 **Tab** 现在还装着两块**投诉单专属**的内容 —— **风险打标**（投诉单由客诉专员在
  // 工单处理页自行打标，§3.1）与**协同记录**（《【930】》§3.3 明写落点是「「风险报备」Tab
  // 「协同记录」块」）。按类型把投诉单挡在外面，这两块就没有地方放。
  // Tab 内各块自己按类型分岔：报备与评估两块只在非投诉单出，打标与协同记录只在投诉单出。
  //
