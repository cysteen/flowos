...
const TAB_TYPE_RESTRICTION: Partial<Record<ProcessTabKey, string[]>> = {
  tech: ['投诉', '咨询'],
  // 风险报备 **只在非投诉单**（基线 v1.20 §4「风险报备」行类型集 ＝ 咨 建 商，※29）。
  // 判据与《【915】风险监控》§1.1 同源：风险 ＝ 这张单**有概率**演变为投诉；已经是投诉单的，
  // 那叫事实不叫风险 —— 它本就在投诉流程里、客诉专员本就在办，再报一次不产生新信息，
  // 只会把评估队列灌满已在处理的单。与「关联售后 ＝ 投」正好互补，两条合起来覆盖四类不重不漏。
  // ⚠️ 原取值 ['投诉'] 是 8 月「报备与协助」（向产研·质量部求助）旧链路留下的，与本 Tab 现在的
  // 业务含义正好相反，2026-09-09 随《【930】》§1.2a 改。
  //
  // 🔴 **本行已整条移除，risk 不再限类型**（2026-09-10，基线 v1.23）。限的是**动作**不是 **Tab**：
  // 「风险报备」这个**动作**的类型集仍是 咨 建 商（登记在 opActionRegistry 的形态表里），
  // 但这个 **Tab** 现在还装着**投诉单专属**的内容 —— **打标入口**（投诉单由客诉专员在
  // 工单处理页自行打标，§3.1；2026-09-20 起它并在「风险标记」块的上半）与**协同记录**
  // （《【930】》§3.3 明写落点是「「风险报备」Tab「协同记录」块」）。
  // 按类型把投诉单挡在外面，这些内容就没有地方放。
  // Tab 内各块自己按类型分岔：报备与评估两块只在非投诉单出，协同记录只在投诉单出；
  // 「风险标记」块四类型都出，块内的打标按钮再按投诉单 + 打标权分岔。
  //
  // 🔴 **不要收成"只给客诉专员"**：本轮不发通知（口径变更），处理人**唯一**能读到
  // 协同意见与打标备注的地方就是这个 Tab。给它关上，页头那两条横幅上的
  // 「查看打标 / 查看协同记录」会指向一个不存在的 Tab，点了没有任何反应。
...
const TAB_ROLE_WRITABLE: Record<ProcessTabKey, readonly RoleKey[]> = {
  ...
  // #43 风险报备：**本 Tab 整块是"发起报备"这一件事**，写动作只有一个，故只需一套判据、
  // 不必拆 canReport / canTag（2026-09-09 拍板）。可写 ＝ 基线 v1.20 §4「风险报备」行取「可用」
  // 的三格：**二线专员 + 二线班组长 + 管理员**（※29）。
  // **客诉专员 / 投诉督导只读** —— 它们是**评估方**，报给自己没有意义；要标风险走风险监控页的
  // 核实打标。**技术支持只读** —— 三线在单上时主责仍是二线（※14 / ※14a），由主责二线报，
  // 是否要给见基线 §7 R16。**质检只读**。一线坐席与工单运营整个 Tab 不渲染（见 TAB_ROLE_DENY）。
  // ⚠️ 原取值 [客诉专员, 投诉督导, 管理员] 对应的是旧口径"写动作＝风险词打标"，那时本 Tab 上
  // 没有报备；改造后打标不在这里，故整体换成报备角色。
  risk: ['agent-l2', 'team-leader', ...MATRIX_ADMIN_ROLES],
